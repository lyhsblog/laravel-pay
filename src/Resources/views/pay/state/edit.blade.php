...
@inject(payService, 'App\Services\PayService')
            ...
            <div class="col-12 col-lg-5">
                <form action="{{route('pay.update', ['id' => $pay->id])}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="status" name="status" value="@error('status'){{old('status')}}@else{{$pay->status}}@enderror" class="form-control @error('status') is-invalid @else is-valid @enderror">
                        @error('status')
